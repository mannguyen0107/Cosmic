/*
 * This file is part of the OdinMS Maple Story Server Copyright (C) 2008 Patrick
 * Huy <patrick.huy@frz.cc> Matthias Butz <matze@odinms.de> Jan Christian Meyer
 * <vimes@odinms.de> This program is free software: you can redistribute it
 * and/or modify it under the terms of the GNU Affero General Public License as
 * published by the Free Software Foundation version 3 as published by the Free
 * Software Foundation. You may not use, modify or distribute this program under
 * any other version of the GNU Affero General Public License. This program is
...
public class Server {
    ...
    private final MonitoredReentrantReadWriteLock wldLock = new MonitoredReentrantReadWriteLock(
            MonitoredLockType.SERVER_WORLDS, true);
    ...
    private final MonitoredReentrantReadWriteLock lgnLock = new MonitoredReentrantReadWriteLock(
            MonitoredLockType.SERVER_LOGIN, true);
     ...
     * lgnWLock.lock(); try { accountCharacterCount.remove(accountid);
     * accountChars.remove(accountid); } finally { lgnWLock.unlock(); } for (World
     * wserv : this.getWorlds()) { wserv.clearAccountCharacterView(accountid);
    ...
    private static void applyAllWorldTransfers(Connection con) throws SQLException {
        try (PreparedStatement ps = con.prepareStatement("SELECT * FROM worldtransfers WHERE completionTime IS NULL",
                ResultSet.TYPE_SCROLL_INSENSITIVE, ResultSet.CONCUR_READ_ONLY); ResultSet rs = ps.executeQuery()) {
    ...
    private synchronized void shutdownInternal(boolean restart) {
         ...
         * System.err.println("FUCK MY LIFE"); } } } for (Channel ch : getAllChannels())
         * { while (ch.getConnectedClients() > 0) { try { Thread.sleep(1000); } catch
         * (InterruptedException ie) { System.err.println("FUCK MY LIFE"); } } }
         */
